...
class InquilinoController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'correo_electronico' => 'required|correo_electronico|unique:inquilinos,correo_electronico',
            'servicios' => 'required|array',
            'servicios.*' => 'exists:servicios,id',
        ]);

        $inquilino = Inquilino::create($request->only('nombre', 'correo_electronico'));
    ...
    public function update(Request $request, $id)
    {
        $request->validate([
            'nombre' => 'required|string|max:255',
            'correo_electronico' => 'required|email|unique:inquilinos,correo_electronico,' . $id,
            'servicios' => 'required|array',
            'servicios.*' => 'exists:servicios,id',
        ]);
